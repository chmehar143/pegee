...
use AuthenticatesUsers;
    ...
    public function login(Request $request) {
        ...
        // Customization: If user status is deactive (-1) return deactive_status error.
        if ($user) {
            if ($user->status == -1) {
                return $this->sendFailedLoginResponse($request, 'auth.deactive_status');
            }

            // Customization: If user status is block (0) return block_status error.
            if ($user->status == 0) {
                return $this->sendFailedLoginResponse($request, 'auth.block_status');
            }
        }
