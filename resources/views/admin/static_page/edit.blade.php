    ...
    <div class="row">
        <div class="col-md-12 col-xs-12">
            <div class="panel panel-default">
                <div class="panel-heading">Update {{ $static_page->page_name }} Page</div>
                <div class="panel-body">
                    <form class="" method="POST" action="{{ route('static_page.update', ['id' => $static_page->id]) }}">
                        {{ method_field('PATCH') }}
                        {{ csrf_field() }}
                        ...
                        <div class="form-group{{ $errors->has('page_name') ? ' has-error' : '' }}">
                            ...
                            <input id="page-name" type="text" class="form-control" name="page_name" value="{{ $errors->has('page_name') ? old('page_name') : $static_page->page_name }}" required autofocus>
                        ...
                        <div class="form-group{{ $errors->has('page_show') ? ' has-error' : '' }}">
                            ...
                            <select name="page_show" id="page_show" class="form-control" >
                                ...
                                <option value="{{$key}}" {{ $static_page->page_show == $key ? 'selected' : ''}} >{{$show_page}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group{{ $errors->has('page_description') ? ' has-error' : '' }}">
                            ...
                            <textarea id="page-description" class="form-control" rows="5" name="page_description">{{ $errors->has('page_description') ? old('page_description') : $static_page->page_description }}</textarea>
